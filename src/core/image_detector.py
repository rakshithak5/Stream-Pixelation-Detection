...
class ImageDetector:
    ...
    def analyze(self, image: np.ndarray) -> Dict:
        """
        Hybrid MVAD-Primary decision logic with corroboration guard.

        Decision logic:
          1. MVAD > threshold AND at least one corroborating signal → ARTIFACT
          2. BRISQUE > threshold alone → ARTIFACT (strong blocking)
          3. Tier1 > 0.50 alone → ARTIFACT (obvious spatial blocking)
          4. Weighted 60/40 fallback → borderline cases

        Corroborating signals (any one is sufficient):
          - Edge > 0.05   (some boundary pairing detected)
          - BRISQUE > 30  (some quality degradation)
          - Grid > 0.05   (some periodicity detected)
          - Tier1 > 0.15  (some spatial signal overall)
        """
        # ── Compute all signals ──────────────────────────────────────────────
        ...
        corroborating_signals = {
            ...
            'tier1_spatial': tier1_spatial > 0.035,  # edge+grid combined
        }
        ...
        # ── Hybrid decision ──────────────────────────────────────────────────
        if mvad_score > self.mvad_threshold and has_corroboration:
            # MVAD + at least one traditional signal agree
            artifact_detected = True
            confidence        = mvad_score
            decision_maker    = f'MVAD+{corroborating_signal_name}'

        elif mvad_score > self.mvad_threshold and not has_corroboration:
        ...
        return {
            ...
            'voting': {
                'artifact_votes': artifact_votes,
                'clean_votes':    4 - artifact_votes,
            ...
            'note': (
                ...
                f'BRISQUE={brisque_score:.1f}, Corroboration={has_corroboration}'
            ),
        }
